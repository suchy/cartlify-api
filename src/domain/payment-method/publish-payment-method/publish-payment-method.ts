...
export type publishPaymentMethod = (params: {
  context: Context;
  paymentMethodId: string;
}) => Promise<PaymentMethod>;

export type PublishPaymentMethodFactory = (dependencies: {
...
export const PublishPaymentMethodFactory: PublishPaymentMethodFactory = ({
  checkPermissions,
  ...
  const publishedPaymentMethod = paymentMethodService.publish();

  const isUpdated = await paymentMethodsRepository.update(
    query,
    publishedPaymentMethod
  );
